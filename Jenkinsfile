  ...
  stages {
    ...
    stage('Build') {
      ...
      steps {
        container('docker') {
          // Build new image
          sh "until docker ps; do sleep 3; done && docker build -t lukyr/argocd-demo:${env.GIT_COMMIT} ."
          // Publish new image
      ...
      steps {
        container('tools') {
          ...
          dir("argocd-demo-deploy") {
            sh "cd ./e2e && kustomize edit set image lukyr/argocd-demo:${env.GIT_COMMIT}"
    ...
    stage('Deploy to Prod') {
      steps {
        ...
        container('tools') {
          dir("argocd-demo-deploy") {
            sh "cd ./prod && kustomize edit set image lukyr/argocd-demo:${env.GIT_COMMIT}"
            sh "git commit -am 'Publish new version' && git push || echo 'no changes'"
          }
        }
      }
